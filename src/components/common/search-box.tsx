	...
	({ className, onSubmit, onClear, ...rest }, ref) => {
		...
		return (
			...
			>
				...
				<button
					type="button"
					className="outline-none text-2xl md:text-3xl text-gray-400 absolute top-0 end-0 w-6 md:w-9 h-full flex items-center justify-center transition duration-200 ease-in-out hover:text-heading focus:outline-none"
					onClick={onClear}
				>
					<p className=" text-[19px] mr-7">clear</p>
				</button>
			</form>
		);
	}
